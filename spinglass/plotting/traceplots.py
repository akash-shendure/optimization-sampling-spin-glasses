# optimizer/sampler trace overlays — energy or grad_norm vs step
import numpy as np
import matplotlib.pyplot as plt

# overlay a single optimizer run's trace; if y='energy' also draw running best
def plot_optimizer_trace(record, x="step", y="energy", best=True, ax=None, title=None):
    ...
    # only meaningful for energy traces — grad_norm has no "best" notion
...
# overlay multiple sampler chains' traces; one line per record (chain)
def plot_sampler_trace(records, observable="energy", x="step", ax=None, title=None):
    ...
    for record in records:
        trace = record["trace"]
        # silently skip records that don't carry this observable
        if trace is None or observable not in trace:
            continue
        xx = np.asarray(trace[x], dtype=np.float64)
        yy = np.asarray(trace[observable], dtype=np.float64)
        # prefer chain id for legend; fall back to algorithm name for single-chain runs
    ...
    # only show legend when it stays readable
