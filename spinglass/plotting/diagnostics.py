# mcmc diagnostic plots: trace, acf, rank histogram, pair scatter, pair matrix
import numpy as np
import matplotlib.pyplot as plt

from ..diagnostics.mcmc_stats import acf

# overlay per-chain time series; accepts (n_chains, n_draws) or a single 1d chain
def plot_trace(chains_2d, x=None, ax=None, title=None, ylabel="value", alpha=0.9):
    ...
    if title is not None:
        ax.set_title(title)
    # only show legend for small chain counts — clutter otherwise
    if chains.shape[0] <= 8:
        ax.legend()
    return ax.figure, ax

# per-chain autocorrelation up to max_lag; fixed y-range exposes the decay shape
def plot_acf(chains_2d, max_lag=100, ax=None, title=None, alpha=0.9):
    ...
    ax.set_xlabel("lag")
    ax.set_ylabel("acf")
    ax.set_ylim(-0.2, 1.0)  # why: keep all panels on the same scale for easy comparison
    if title is not None:
        ax.set_title(title)
    if chains.shape[0] <= 8:
        ax.legend()
...
# rank-normalize pooled draws then histogram per chain — uniform under mixing
def plot_rank_histogram(chains_2d, bins=20, ax=None, title=None, density=True):
    ...
    # double-argsort gives the rank of each element within the pooled sample
...
# scatter of paired observables per chain (e.g. energy vs |q|)
...
# corner-style matrix: diagonal histograms, lower triangle scatters, upper hidden
def plot_pair_matrix(data_dict, names=None, title=None, n_per_series=800, alpha=0.3, s=6):
    ...
    # subsample evenly along the draw axis to keep scatter readable
    idx = np.linspace(0, n_draws - 1, min(n_per_series, n_draws)).astype(int)

    # figure setup: k x k grid sized so each panel is ~3.2 inches square
    k = len(names)
    fig, axes = plt.subplots(k, k, figsize=(3.2 * k, 3.2 * k), squeeze=False)
    for i in range(k):
        for j in range(k):
            ax = axes[i, j]
            # upper triangle is redundant — hide it
            if i < j:
                ax.axis("off")
                continue
            if i == j:
                # diagonal: marginal histogram pooled across chains
                pooled = arrays[i][:, idx].reshape(-1)
                ax.hist(pooled, bins=25, density=True, alpha=0.8)
            else:
                # off-diagonal: one scatter per chain
                for c in range(n_chains):
                    ax.scatter(arrays[j][c, idx], arrays[i][c, idx], s=s, alpha=alpha)
            # only label edge axes — keep the grid clean
            if i == k - 1:
...
# promote 1d arrays to (1, n) so downstream code can iterate uniformly
def _to_2d(x):
    x = np.asarray(x, dtype=np.float64)
    if x.ndim == 1:
        return x[None, :]
