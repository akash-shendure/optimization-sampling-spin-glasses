# replica-overlap plots: P(q) histograms, <|q|> vs beta, and <|q|> vs mean energy
import numpy as np
import matplotlib.pyplot as plt

# single P(q) histogram from a precomputed overlap row (bin_edges + density)
def plot_overlap_histogram(overlap_rows, ax=None, title=None, color="#1f77b4"):
    if not overlap_rows:
        raise ValueError("overlap_rows must be non-empty")
    # accept either a list (take first) or a single row dict
    ...
    ax.set_ylabel("P(q)")
    ax.set_xlim(-1.05, 1.05)  # why: q lives in [-1, 1]; small pad keeps bar edges visible
    if title is not None:
        ax.set_title(title)
    return ax.figure, ax

# grid of P(q) panels, one per beta value, with shared axes for direct comparison
def plot_overlap_histograms_by_beta(overlap_rows, beta_key="algorithm_beta", ncols=3, title=None):
    ...
    # sharex/sharey: every panel reads on the same q and P(q) scale
    ...
    # hide unused trailing axes when grid is not a perfect rectangle
    for ax in axes[n:]:
        ax.axis("off")
    if title is not None:
        fig.suptitle(title)
...
# <|q|> as a function of beta — the order-parameter sweep
def plot_mean_abs_q_curve(overlap_rows, beta_key="algorithm_beta", ax=None, title=None, marker="o"):
    ...
    ax.set_ylabel("<|q|>")
    ax.set_ylim(-0.05, 1.05)  # why: |q| in [0, 1]; pad so endpoints aren't clipped
    if title is not None:
        ax.set_title(title)
    return ax.figure, ax

# parametric curve (<|q|>, <E>) labelled by beta — overlap vs energy plane
def plot_overlap_vs_energy(overlap_rows, energy_rows, beta_key="algorithm_beta", ax=None, title=None):
    # join overlap and energy tables by beta key
    ...
    # annotate each marker with its beta — easier than a colorbar for ~10 points
