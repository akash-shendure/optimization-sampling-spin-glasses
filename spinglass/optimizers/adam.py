# adam optimizer on the relaxed smooth surrogate H_tilde(x); optional projection back to {-1,+1}
import numpy as np

from ..utils.records import append_trace, finalize_trace, init_trace, now
from ..utils.rng import make_rng

# adaptive moments on x in R^n; rounds via tanh-sign at the end if asked
class AdamOptimizer:
    # standard adam hyperparams; eps guards the sqrt(vhat) divide
    ...
    # main descent loop: trace, check target, then adam step
    def run(
        self,
        x0=None,
        n_steps=1000,
        ...
        for step in range(n_steps + 1):
            elapsed = now() - start
            # optional projected energy so we can compare relaxed vs discrete
            ...
            if target_energy is not None and hit_step is None and best_energy <= target_energy:
                hit_step = step
                hit_time = elapsed
            # trace at step n_steps then exit (no further update needed)
            if step == n_steps:
                break
            ...
            mhat = m / (1.0 - self.beta1 ** t)  # bias-correct early steps
            vhat = v / (1.0 - self.beta2 ** t)
            x -= self.lr * mhat / (np.sqrt(vhat) + self.eps)
            energy, grad = self.hamiltonian.energy_and_grad(x)
            if energy < best_energy:
                best_energy = float(energy)
                best_x = x.copy()

        # finalize: optionally project final/best x back to discrete spins
