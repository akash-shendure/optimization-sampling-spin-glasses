# greedy single-flip descent: pick site i with most-negative dE = 2 s_i h_i until none remain
import numpy as np
...
class GreedySpinDescent:
    ...
    def run(self, s0=None, n_steps=None, trace_every=1, target_energy=None, store_states=False):
        ...
        trace = init_trace()
        state_trace = []
        # default budget = n (each site once on average)
        ...
        for step in range(n_max + 1):
            ...
            if step == n_max:
                break
            # dE_i = 2 s_i h_i across all sites; only negatives are improving
            dE = self.hamiltonian.delta_energy_all(s, h=h)
            negative = np.flatnonzero(dE < 0.0)
            # local minimum: no improving flip exists
            if negative.size == 0:
                break
        ...
        summary = {
            ...
            "task": "optimization",
            "space": "discrete",
            # actual steps taken can be < n_max if we hit a local min early
