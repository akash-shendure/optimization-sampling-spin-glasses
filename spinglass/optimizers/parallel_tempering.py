# parallel tempering as a search heuristic: n_replica metropolis chains along a beta ladder with adjacent swaps
import numpy as np
...
class ParallelTemperingOptimizer:
    # betas must be 1d with >= 2 entries (ladder needs at least two rungs to swap)
    ...
    # main loop: per-step MH sweep across replicas, then optional swap pass
        ...
        for step in range(n_steps + 1):
            ...
            if step == 0 or step % trace_every == 0:
                append_trace(
                    ...
                    # max(1, ...) guards step=0 trace before any attempts
            ...
            # MH sweep: one random-site flip per replica at its own beta
