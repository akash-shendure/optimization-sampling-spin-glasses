# simulated annealing: single-site metropolis with a beta(step) schedule cooling toward low-energy basins
import numpy as np
...
class SimulatedAnnealing:
    ...
    def _beta(self, step, n_steps):
        ...
        # hold final beta if step overruns the schedule
        idx = min(step, schedule.size - 1)
        return float(schedule[idx])

    # main MH loop with annealed beta and cached local fields
    def run(self, s0=None, n_steps=1000, trace_every=1, target_energy=None, store_states=False):
        ...
        for step in range(n_steps + 1):
            ...
            if step == 0 or step % trace_every == 0:
                # max(1, step) guards the divide at step=0
            ...
            if step == n_steps:
                break
            # MH accept: random site, dE = 2 s_i h_i, accept downhill or with prob exp(-beta dE)
