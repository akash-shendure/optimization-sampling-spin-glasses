# metropolis-adjusted langevin (MALA): ULA proposal + MH correction for unbiased sampling
import numpy as np
...
class MALASampler:
    ...
    # ULA drift mean used by the gaussian proposal
    def _proposal_mean(self, x, grad):
        return x - self.step_size * self.beta * grad

    # log q(to | from) up to constant; variance 2*step => -|r|^2 / (4 step)  # why MH correction
    ...
    # n_steps MALA iterations with optional projection logging
    def run(
        self,
        x0=None,
        n_steps=1000,
        ...
        # init x, energy/grad, and cached log-target
        ...
        for step in range(n_steps + 1):
            ...
            # MH log-accept: log pi(y)/pi(x) + log q(x|y) - log q(y|x)  # why MALA correction
            log_alpha = (
                prop_logp
