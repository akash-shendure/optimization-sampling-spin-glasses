# shows parallel tempering: a three-rung beta ladder swapping replicas to mix across barriers
from pathlib import Path

import numpy as np
from manim import *

# scene-local manim cache so each animation owns its render artifacts
...
config.background_color = WHITE  # white bg for print/figures

# palette: black ink, blue water (thermal), red walker, purple swap event
FG = BLACK
WATER_FILL = BLUE_C
WATER_LINE = BLUE_E
PARTICLE = RED_E
SWAP = PURPLE_E  # distinct hue so swap edges read clearly against walker trails

EQN_SIZE = 38
LABEL_SIZE = 28

# three stacked panels sharing one x-domain
X_MIN, X_MAX = -5.0, 5.0
PANEL_WIDTH = 10.2
PANEL_HEIGHT = 1.80
PANEL_Y = (2.55, 0.00, -2.55)  # top = hot, middle = warm, bottom = cold
PANEL_X_SHIFT = 0.0

# smooth multi-well envelope — coarse shape of E(x)
def _envelope(x):
    return (
...
# incommensurate sine modes that give the curve its rugged metastable bumps
_MODES = (
    (0.85,  0.70, 0.32),
    (1.35,  2.10, 0.26),
    (1.90, -0.30, 0.22),
...
# high-frequency roughness on top of the envelope
def _rugged(x):
...
# full toy energy: smooth wells + glassy fluctuations
def energy(x):
    x = np.asarray(x, dtype=float)
    return _envelope(x) + _rugged(x)

# scalar wrapper for manim lambdas
def _energy_scalar(x):
    return float(energy(np.array([float(x)]))[0])

# precomputed probe grid for water level + hop validity
...
# clamp the "water" level so visualizations don't run off-axis
E_FLOOR = _E_MIN - 0.05
LEVEL_CAP = _E_MAX + 0.65
THERMAL_C = 1.60  # cartoon: level ~ floor + C/beta

# the three rungs of the beta ladder
BETA_HOT = 0.35
BETA_WARM = 1.00
BETA_COLD = 2.60

# map beta -> thermal water level on the shared landscape
def level_from_beta(beta):
    return float(min(E_FLOOR + THERMAL_C / max(float(beta), 1e-3), LEVEL_CAP))

# small, thin axes used in each panel of the stack — no ticks, conceptual only
...
# E(x) curve drawn into a single panel; thinner than the solo-scene version
def _panel_curve(ax):
    return ax.plot(
...
# fill below the water level — one polygon per accessible basin
def _water_fill(ax, level, n_samples=1200):
    ...
    # interp where the curve crosses `level` between two probe points
    ...
    # sweep, collecting each contiguous below-level run as a closed polygon
    i = 0
    while i < n - 1:
        ...
        # close polygon along the level line back to its left edge
...
# top edge of the water — straight segments wherever E(x) dips below level
...
# reject hops that pass through the curve — chord must clear the landscape
...
# 1/sqrt(gap) — cartoon density-of-states weighting near a level
...
# stateful per-rung walker — owns its rng, visit-counts, and current x
class _Walker:
    # one walker per beta; sigma + global_prob set by rung (hot proposes wider)
    def __init__(
    ...
    # bucket x into one of n_bins (uniform on [x_lo, x_hi])
    def _bin(self, xv):
        f = (xv - self.x_lo) / (self.x_hi - self.x_lo)
        return int(np.clip(f * self.n_bins, 0, self.n_bins - 1))

    # log weight = log(physics density) - repulsion * visits to that bin
    ...
    # one accepted move; capped proposal budget keeps animation timing stable
    def step(self, max_props=500):
        lw_curr = self._log_weight(self.x, self.visits[self._bin(self.x)])
        for _ in range(max_props):
            # mix local gaussian with occasional uniform jumps (hot rung only)
    ...
    # teleport (used by swap moves) — also bumps visits to avoid immediate undo
    def set_x(self, new_x):
        self.x = float(new_x)
        self.visits[self._bin(self.x)] += 1.0

# per-rung proposal tuning: hot rung jumps far + often globally, cold stays local
_GLOBAL_PROBS = (0.25, 0.12, 0.00)
_REPULSIONS = (1.00, 0.80, 0.80)
_SIGMAS = (0.55, 0.30, 0.18)

# core PT loop: each rung evolves; every swap_every steps try an adjacent swap
def _run_pt(betas, levels, x0s, n_steps, swap_every, seed):
    ...
    # separate rng for swap decisions so per-walker rngs stay reproducible
    master_rng = np.random.default_rng(seed + 99)

    # trajectories[k] = list of (step, x, was_swap) for replica k
    trajectories = [[(0, walkers[k].x, False)] for k in range(3)]
    swap_events = []
    pair_counter = 0

    for step in range(1, n_steps + 1):
        # local move on every rung
        for k in range(3):
            walkers[k].step()
        for k in range(3):
            trajectories[k].append((step, walkers[k].x, False))

        # attempt a swap on adjacent pairs, alternating (0,1) and (1,2)
        if step % swap_every == 0:
            ...
            # both x's must be admissible at the partner's water level
            if E_i >= levels[j] or E_j >= levels[i]:
                continue

            # standard PT acceptance: delta = (beta_i - beta_j)(E_i - E_j)
            delta = (betas[i] - betas[j]) * (E_i - E_j)
            if master_rng.random() < np.exp(min(0.0, delta)):
                walkers[i].set_x(x_j)
                walkers[j].set_x(x_i)
                # overwrite last entry with swap-flagged endpoints for trail rendering
...
# visual styling for swap segments — dashed purple to distinguish from local moves
SWAP_STROKE_WIDTH = 3.0
SWAP_DASH_LENGTH = 0.14
SWAP_ARROW_TIP_LENGTH = 0.22

# build the trail+dot for one rung — segments revealed in lockstep with sim time
def _build_trail_and_particle(ax, traj, step_dt,
    ...
    # one segment per step; swaps drawn as dashed purple, local moves as solid red
    segments = []
    for i in range(n - 1):
        if is_swap[i + 1]:
            seg = DashedLine(
        ...
        seg.set_stroke(opacity=0.0)  # hidden until updater reveals it
        segments.append(seg)
    ...
    # reveals each segment as sim time crosses its step boundary
    def trail_updater(mob, dt):
        ...
        # finish any earlier segments fully
        ...
        # draw the current segment partially — swap segs pop in at 30% so the dashed line reads
        if idx < n - 1:
            seg = segments[idx]
            if is_swap[idx + 1]:
                if frac > 0.30:
    ...
    # moves the dot; for swap steps it teleports at the half-frame mark
    def particle_updater(mob, dt):
        ...
        if idx >= n - 1:
            ...
            if is_swap[idx + 1]:
                # snap mid-step so motion reads as instantaneous teleport
                if frac < 0.5:
                    mob.move_to(pts[idx])
                else:
...
# cross-panel double-arrow shown at swap times — visually ties the two rungs
def _make_cross_panel_arrow(ax_i, ax_j, x_i_new, x_j_new):
    ...
    # start invisible — the schedule updater fades it in at its event time
    arrow.set_stroke(opacity=0.0)
    arrow.set_fill(opacity=0.0)
    return arrow

# orchestrates fade-in/hold/fade-out of each swap arrow on its scheduled step
def _swap_arrow_updater(arrow_schedule, step_dt,
    ...
    def updater(mob, dt):
        ...
        for (step, arrow) in arrow_schedule:
            # arrow begins fading in just before its swap step lands
            ...
            if dt_local < 0 or dt_local > total_life:
                ...
                arrow.set_fill(opacity=0.0)
                continue
            # three-piece envelope: linear in, hold at 1, linear out
            if dt_local < fade_in:
...
# pt simulation length and timing
N_STEPS = 120
STEP_DT = 0.10
SWAP_EVERY = 6  # how often to attempt swaps in sim steps
SEED = 404

# scene 1: full ladder runs side-by-side with cross-panel swap arrows
class Scene1_LadderWithSwaps(Scene):
    def construct(self):
        # build the three rungs of the ladder: hot, warm, cold
        ...
        # one axes+curve+water+label set per rung
        ...
        # introduce the ladder: axes+labels, then stagger the curves, then water
        self.play(
        ...
        # lagged Create so eye reads top->bottom rather than all at once
        self.play(
        ...
        # seed all three walkers at the cold rung's deepest basin
        # — they start "stuck" together so swaps visibly free the cold replica
        ...
        # run the full PT chain offline; trajectories drive the animation
        trajectories, swap_events = _run_pt(
            betas=betas, levels=levels, x0s=x0s,
            n_steps=N_STEPS, swap_every=SWAP_EVERY, seed=SEED,
        )

        # build trails + particles per rung; track max duration so we wait long enough
        trails, particles = [], []
        t_upds, p_upds = [], []
        total_time = 0.0
        for k in range(3):
        ...
        # one DoubleArrow per accepted swap, scheduled by step
        arrow_schedule = []
        ...
        # add trails/particles/arrows to the scene (hidden by default)
        for tr, p in zip(trails, particles):
            self.add(tr, p)
        self.add(arrow_group)
        self.wait(0.3)

        # attach updaters and let the simulation play out
        ...
        # detach so the final frame holds cleanly
        for tr, t_up in zip(trails, t_upds):
