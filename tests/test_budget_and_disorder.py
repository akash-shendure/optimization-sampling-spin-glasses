# Budget.to_n_steps unit conversion + disorder replicate injection / averaging plumbing
import numpy as np

from spinglass import Budget, budget_to_n_steps
from spinglass.experiments.budget import hamiltonian_evals, steps, sweeps
from spinglass.experiments.studies import _inject_replicates, sampling_beta_sweep
from spinglass import IsingFerromagnet2D

# kind='steps' should pass the raw count through unchanged in either space
def test_budget_steps_passes_through():
    b = steps(1234)
    assert b.to_n_steps(n_spins=50, space="discrete") == 1234
    assert b.to_n_steps(n_spins=50, space="relaxed") == 1234

# a sweep = n single-spin updates in discrete, but one vector step in relaxed
def test_budget_sweeps_scales_discrete_by_n():
    b = sweeps(3)
    assert b.to_n_steps(n_spins=64, space="discrete") == 192
    assert b.to_n_steps(n_spins=64, space="relaxed") == 3

# hamiltonian_evals matches the cost model: n flips per H-eval in discrete, 1 in relaxed
def test_budget_hamiltonian_evals_matches_compute():
    b = hamiltonian_evals(10)
    assert b.to_n_steps(n_spins=64, space="discrete") == 640
    assert b.to_n_steps(n_spins=64, space="relaxed") == 10

# unknown kind and non-positive value should both fail loudly at construction
def test_budget_rejects_bad_inputs():
    try:
        Budget(kind="nope", value=10)
    except ValueError:
        pass
    else:
        assert False, "Budget should reject unknown kind"
    try:
        Budget(kind="steps", value=-1)
    except ValueError:
        pass
    else:
        assert False, "Budget should reject non-positive value"

# bare int input means "this many steps", unchanged across spaces
def test_budget_to_n_steps_accepts_plain_int():
    assert budget_to_n_steps(500, n_spins=16, space="discrete") == 500
    assert budget_to_n_steps(500, n_spins=16, space="relaxed") == 500

# n_disorders > 1 fans out a _disorder_id axis for the study grid
def test_inject_replicates_generates_n_disorders():
    out = _inject_replicates({"L": [8]}, n_disorders=4)
    assert "_disorder_id" in out
    assert out["_disorder_id"] == [0, 1, 2, 3]

# n_disorders == 1 should be a no-op (don't pollute the param grid)
def test_inject_replicates_noop_when_single():
    out = _inject_replicates({"L": [8]}, n_disorders=1)
    assert "_disorder_id" not in out
    assert out == {"L": [8]}

# full sampling sweep: fan over disorders, group by beta, check thermal ordering
def test_sampling_sweep_fans_over_disorders_and_averages():
    betas = [0.3, 0.8]
    res = sampling_beta_sweep(
        IsingFerromagnet2D,
        model_kwargs={"L": [6]},
        betas=betas,
        n_chains=2,
        n_steps=200,
        burn_in=40,
        trace_every=5,
        n_disorders=2,
    )
    # 2 betas * 2 disorders * 2 chains = 8 records
    assert len(res["records"]) == 8
    assert len(res["grouped"]) == 2
    assert res["n_disorders"] == 2
    rows = sorted(res["grouped"], key=lambda r: r["algorithm_beta"])
    # higher beta -> lower mean energy on the ferromagnet  # why: physical sanity check
    assert rows[0]["mean_energy"] > rows[-1]["mean_energy"]

# Budget integration: 3 sweeps on L=6 (n=36) discrete -> 3*36 = 108 steps
def test_sampling_sweep_with_budget_honors_sweeps_convention():
    res = sampling_beta_sweep(
        IsingFerromagnet2D,
        model_kwargs={"L": [6]},
        betas=[0.5],
        n_chains=1,
        budget=sweeps(3),
        burn_in=0,
        trace_every=1,
        n_disorders=1,
    )
    assert res["effective_n_steps"] == 3 * 36

if __name__ == "__main__":
    test_budget_steps_passes_through()
    test_budget_sweeps_scales_discrete_by_n()
    test_budget_hamiltonian_evals_matches_compute()
    test_budget_rejects_bad_inputs()
    test_budget_to_n_steps_accepts_plain_int()
    test_inject_replicates_generates_n_disorders()
    test_inject_replicates_noop_when_single()
    test_sampling_sweep_fans_over_disorders_and_averages()
    test_sampling_sweep_with_budget_honors_sweeps_convention()
    print("test_budget_and_disorder OK")
