# early diagnostic figures: metropolis on 2d ising across sizes and betas
...
N_CHAINS = 4
TRACE_EVERY = 10
BUDGET = sweeps(200)  # why: equal wall-budget across sizes for fair scaling
D = 100               # why: 100 disorder seeds gives tight error bars
N_BETAS = 40
...
# same shape as above, but for the replica-overlap |q| summary
...
def _dress_four_panels(axes):
    ...
    axes[1, 1].set_ylim(-0.05, 1.05)  # why: |q| in [0,1]

    for ax in axes.ravel():
        ax.tick_params(labelsize=FS)

# draw one (size's) curves on the four diagnostic panels
def _plot_four(axes, stats, overlap_stats, color, marker, label, show_eb=True):
    ...
    # pull mean column for a metric
    def _v(key):
        return np.array([r.get(f"{key}_mean", np.nan) for r in stats])

    # pull std column; None if error bars suppressed
    def _e(key):
        if not show_eb:
            return None
        return np.array([r.get(f"{key}_std", 0.0) for r in stats])

    # gibbs has no acceptance rate; skip cleanly if all-nan
...
# top-level driver: sweep sizes, then build the scaling figure
def main():
    ...
    # size sweep: one sampling_beta_sweep call per L
    scaling = {}
    for L in SIZES:
        tag = f"L={L}"
        log(f"\n  [{tag}]")
    ...
    # figure assembly: per-size stats, then with/without error bars
    log("\n  Generating figure ...")
    ...
    # render both variants so the report can pick the cleaner one
