# early sampling figures: rhat/ess/tau_int/|q| for all five samplers on 2d ising
...
N_CHAINS = 4
TRACE_EVERY = 10
BUDGET = sweeps(200)  # why: equal wall-budget across samplers
D = 100
N_BETAS = 40
...
# (name, class, search space, sampler kwargs)
ALL_SAMPLERS = [
...
SELECTED_NAMES = ["Metropolis", "Gibbs"]  # subset highlighted in narrower figures
...
# same shape, but for the replica-overlap |q| summary
...
# draw one (sampler's) curves on the four diagnostic panels
def _plot_four(axes, stats, overlap_stats, color, marker, label, show_eb=True):
    ...
    # gibbs has no rhat in some cases; skip cleanly if all-nan
    rh = _v("energy_rhat")
...
# top-level driver: run all five samplers, then build the comparison figures
def main():
    ...
    # run each sampler at the matched wall-budget
    results = {}
    ...
    # figure assembly: only the _all variant is emitted; SELECTED_NAMES path is skipped
    log("\n  Generating figures ...")
    figure_groups = [
